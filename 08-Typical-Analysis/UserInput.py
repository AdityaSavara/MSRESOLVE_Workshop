...
UserChoices['inputFiles']['referencePatternsFileNamesList'] = ['ExtractedReferencePattern.tsv'] #enter the file name of the file containing reference information. tsv is tab-separated, csv is comma separated. tsv supports commas in molecule names.
...
UserChoices['inputFiles']['dataToAnalyzeFileName'] = 'Collected_GasPulse.csv'	#enter the file name with raw mass spectrometer data
...
UserChoices['specificMolecules']['chosenMoleculesNames'] = ['Acetylene', 'Ethylene', 'Ethane', 'ethanol']
...
UserChoices['linearBaselineCorrectionSemiAutomatic']['massesToBackgroundCorrect'] = []			#mflist: enter mass list delimited With commas [m1, m2, m3]. Leave blank for all masses.
# to apply a uniform time range to all fragments, only insert one time range as such [[x,y]]
UserChoices['linearBaselineCorrectionSemiAutomatic']['earlyBaselineTimes'] = [[]]	# to apply different times for each fragment enter time pairs as such [[x,y],[z,w]..]
# to apply a uniform time range to all fragments, only insert one time range [[x,y]]
UserChoices['linearBaselineCorrectionSemiAutomatic']['lateBaselineTimes'] = [[]]	#if you do not wish to enter a second time list enter 0's [[0,0],[0,0]...] or [[0,0]]
...
UserChoices['tuningCorrection']['referenceFileStandardTuningAndForm'] = ['ConvertedSpectra1.tsv', 'xyyy'] #optional: Provides tuningCorrectionIntensity feature. Must include csv file and form. Example: ['NISTRef.csv', 'xyyy'] .     Will automatically be used for ReferenceFileExistingTuning if that variable is blank and if createMixedTuningPattern=True.
...
UserChoices['applyReferenceMassFragmentsThresholds']['referenceMassFragmentFilterThreshold'] = [10.2]
UserChoices['applyReferenceMassFragmentsThresholds']['referenceSignificantFragmentThresholds'] = [10.0]
...
UserChoices['dataAnalysisMethods']['implicitSLScorrection'] = True #recommended when doing SLS Unique with uncertainties.
